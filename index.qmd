---
title: "About me"
about:
  template: marquee
  image-width: 9em
  image-shape: round
date-modified: last-modified
description: |
  [Blog](/blog/){.btn .btn-outline-primary}
  [Zenn](/zenn/){.btn .btn-outline-primary}
  [Slides](/slides/){.btn .btn-outline-primary}
  [Resume](/resume/){.btn .btn-outline-primary}
  [Resume (EN)](/en/resume/){.btn .btn-outline-primary}
  [GitHub](https://github.com/i9wa4){.btn .btn-outline-primary}
  [LinkedIn](https://www.linkedin.com/in/daikm){.btn .btn-outline-primary}
  [X](https://x.com/i9wa4_){.btn .btn-outline-primary}
image: "https://github.com/i9wa4.png"
toc: false
---

<!--
Badge generation (style=social):

1. Basic format:
   https://img.shields.io/badge/{LABEL}-{MESSAGE}-x.svg?style=social&logo={ICON}
   - {LABEL}: Service name (GitHub, X, etc.)
   - {MESSAGE}: User ID
   - -x: Dummy color (ignored in social style but required as separator)
   - Escape underscore: _ -> __

2. Icons:
   - Simple Icons: https://simpleicons.org
   - Available: github, x, bluesky, zenn
   - NOT available: linkedin (removed due to brand guidelines)

3. LinkedIn workaround (Iconify + base64):
   Step 1: Get SVG from Iconify API
     curl "https://api.iconify.design/ri/linkedin-fill.svg?color=%230A66C2"
   Step 2: Base64 encode
     echo '<svg>...</svg>' | base64 | tr -d '\n'
   Step 3: Use HTML tag (Markdown adds .png suffix, breaking base64)
     <a href="URL"><img src="...?logo=data:image/svg+xml;base64,{BASE64}" alt="LinkedIn"></a>

   Iconify icon search: https://icon-sets.iconify.design
-->

[![Website](https://img.shields.io/badge/Website-i9wa4.github.io-181717?style=flat&logo=googlechrome&logoColor=white)](https://i9wa4.github.io)
[![GitHub](https://img.shields.io/badge/GitHub-i9wa4-181717?style=flat&logo=github&logoColor=white)](https://github.com/i9wa4)
<a href="https://www.linkedin.com/in/daikm"><img src="https://img.shields.io/badge/LinkedIn-daikm-0A66C2?style=flat&logo=data:image/svg+xml;base64," alt="LinkedIn"></a>
[![X](https://img.shields.io/badge/X-i9wa4__-000000?style=flat&logo=x&logoColor=white)](https://x.com/i9wa4_)

```{.yaml}
profile:
  name: uma-chan

  organizations:
    - name: GENDA Inc.
      roles:
        - Data Engineer
        - MLOps Engineer

    - name: PIVOT, Inc.
      roles:
        - Data Engineer

  passions:
    - Vim
    - tmux
    - Databricks
```

[![0-profile-details](https://raw.githubusercontent.com/i9wa4/i9wa4.github.io/gh-summary/transparent/0-profile-details.svg)](https://github.com/vn7n24fzkq/github-profile-summary-cards)
[![3-stats](https://raw.githubusercontent.com/i9wa4/i9wa4.github.io/gh-summary/transparent/3-stats.svg)](https://github.com/vn7n24fzkq/github-profile-summary-cards)[![1-repos-per-language](https://raw.githubusercontent.com/i9wa4/i9wa4.github.io/gh-summary/transparent/1-repos-per-language.svg)](https://github.com/vn7n24fzkq/github-profile-summary-cards)

<div align="left">
  <a href="https://github.com/antonkomarev/github-profile-views-counter">
    <img src="https://komarev.com/ghpvc/?username=i9wa4&label=GitHub_Profile_Views&style=flat"/>
  </a>
</div>
