...
MetaGPT の SOP（エージェント間の役割・手順を構造化）と、2025年に広まった SDD（仕様書を起点に AI にコードを生成させる）は哲学的なルーツが共通。ただし層が異なる: SOP は実行時のマルチエージェント協調、SDD は開発時に仕様書からコードを生成するアプローチ。
...
| ファイル型 | tmux ベース, Agent Teams | 各エージェントが自分の inbox のみ参照 |
...
改善ツール（`/a2a-role-auditor` など）は人間が投げる必要がある — 完全自律ではない。しかし:

1. 人間: 問題に気づいて `/a2a-role-auditor` を投入
2. エージェント: ロールテンプレートを監査・問題を特定・パッチを提案
3. 人間: 承認
4. エージェント: 修正を適用・検証
