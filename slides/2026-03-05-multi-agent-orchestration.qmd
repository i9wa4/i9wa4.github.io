...
### 3.4. Anthropic 5 パターンと本システムの対応

Anthropic "Building Effective Agents" (2024) の 5 パターン分類と本システムの実装例

| パターン             | 概要                               | 本システムでの実装                                       |
| -------------------- | ---------------------------------- | -------------------------------------------------------- |
| Prompt Chaining      | LLM の出力を次の LLM に連鎖        | worker → critic → guardian の順次レビュー               |
| Routing              | 入力を分類して専門ハンドラへ振り分け | orchestrator が難易度で worker / worker-alt を選択      |
| Parallelization      | 複数エージェントを並列実行          | worker と worker-alt の並列タスク分担                    |
| Orchestrator-Workers | orchestrator が worker に委譲      | orchestrator → worker / worker-alt / critic / guardian   |
| Evaluator-Optimizer  | 生成と評価のループ                  | worker 生成 + critic 評価 + guardian 承認のループ        |
