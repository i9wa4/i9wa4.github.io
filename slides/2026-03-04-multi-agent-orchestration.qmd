...
### 3.1.1. openclaw とは（補足）

**openclaw** (<https://github.com/openclaw/openclaw>) — 259,000+ stars

"Your own personal AI assistant. Any OS. Any Platform."

マルチエージェントではない — パーソナル AI アシスタント

- 20+ メッセージチャンネル（WhatsApp, Slack, Discord 等）から単一 Gateway へ接続
- Gateway がセッション管理とルーティングを担い、エージェントワークスペースを分離
- ここで紹介する理由: 話題性が非常に高く（259,000+ stars）、AI アシスタントのアーキテクチャの参考事例として

```{mermaid}
graph LR
  CH["メッセージチャンネル<br/>WhatsApp / Slack / Discord / ...<br/>(20+ 種類)"]
  GW["Gateway<br/>ルーティング・セッション管理<br/>(WS :18789)"]
  AGENT["エージェント<br/>Pi ランタイム<br/>(ワークスペース分離)"]
  MODELS["AI プロバイダー<br/>OpenAI / Anthropic / ..."]
  TOOLS["ツール<br/>browser / canvas / exec"]
  CH --> GW
  GW --> AGENT
  AGENT --> MODELS
  AGENT --> TOOLS
  classDef chanClass  fill:#4A90D9,color:#fff,stroke:#2C5F8A
  classDef gwClass    fill:#F5A623,color:#fff,stroke:#C47A00
  classDef agClass    fill:#7BC67E,color:#fff,stroke:#4A8C4E
  classDef aiClass    fill:#9B59B6,color:#fff,stroke:#6C3483
  classDef toolClass  fill:#E85D7A,color:#fff,stroke:#B23358
  class CH chanClass
  class GW gwClass
  class AGENT agClass
  class MODELS aiClass
...
| fork（tmux ベース） | ペインタイトルをノード名として検出し、ファイルベースのメッセージ経由で複数エージェントをルーティング | 完全独立・透明性・LLM 種類問わず  | セットアップが必要、自前でメッセージ管理                |
...
| worker       | Claude Sonnet 4.6       | 実装・調査・コード生成                                   |
| worker-alt   | Claude Sonnet 4.6       | 実装・調査（並列実行・単純タスク専用）                   |
| critic       | Claude Sonnet 4.6       | 批判的分析・品質チェック                                 |
| guardian     | gpt-5.3-codex           | QA レビュー・承認ゲート（boss への転送判断）             |
| messenger    | Claude Sonnet 4.6       | ユーザーとのインターフェース・メッセージ中継             |
:::
:::::
...
graph TD
  ...
  WORKER["worker<br/>Claude Sonnet 4.6"]
  WORKER_ALT["worker-alt<br/>Claude Sonnet 4.6"]
  CRITIC["critic<br/>Claude Sonnet 4.6"]
  GUARD["guardian<br/>gpt-5.3-codex"]
  MESS["messenger<br/>Claude Sonnet 4.6"]
  USER["user<br/>人間"]
  BOSS --> ORC
  ORC --> WORKER
  ORC --> WORKER_ALT
...
tmux-a2a-postman を使うことで、tmux の fork アプローチに標準的なメッセージルーティング層を追加できる
