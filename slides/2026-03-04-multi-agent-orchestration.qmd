...
**前回のおさらい**: Slack でシェアした tmux 活用記事
...
| MetaGPT | 標準手順書 (SOP) 駆動・ソフトウェア会社シミュレーション | <https://github.com/FoundationAgents/MetaGPT> (64,000+ stars) |
...
`va` プリセット (`vde-layout messenger && vde-layout high`) で 2 ウインドウを起動:

```yaml
# Window 1: messenger preset (current-window)
- editor      # vim
- messenger   # Claude Sonnet

# Window 2: high preset (new-window) — 3×2 グリッド
左列: orchestrator / worker / worker-alt  (Claude Sonnet)
右列: critic / guardian / boss            (Sonnet / gpt-5.3-codex / Opus)
```
