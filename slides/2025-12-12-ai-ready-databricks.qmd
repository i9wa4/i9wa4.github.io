---
title: Databricks向けJupyter Kernelでデータサイエンティストの開発環境をAI-Readyにする
author:
  - name: uma-chan
categories:
  - "slides"
  - "genda"
date: 2025-12-xx
date-modified: last-modified
description: |
  [登壇資料] 2025-12-12 イベント名
draft: true
image: "/assets/common/icon_hhkb3_large.jpg"
format:
  revealjs:
    auto-stretch: false
    code-copy: true
    footer: "[2025-12-xx イベント名](https://example.com)"
    include-in-header:
      - ../_includes/hatena-nocomment.html
    incremental: false
    scrollable: true
    slide-level: 3
    slide-number: true
    smaller: false
    theme:
      - simple
      - ../themes/font.scss
    title-prefix: ""
    toc-depth: 2
    toc: false
    height: 1080
    width: 1920
  gfm: default
subtitle: jupyter-databricks-kernel + ガードレールで最強の開発環境を構築
---

## 1. はじめに

### 1.1. 自己紹介

::: {style="display: flex; flex-direction: column; align-items: center; gap: 2em;"}

::: {style="text-align: left;"}

uma-chan (馬渡 大樹 / Mawatari Daiki)

:::

::: {style="text-align: left;"}

|                              |                                     |
| ---                          | ---                                 |
| Data Engineer/MLOps Engineer | [株式会社GENDA](https://genda.jp/)  |

:::

::: {style="text-align: center;"}

好きなものたち

[![Vim](https://img.shields.io/badge/Vim-019733.svg?logo=vim&logoColor=white){height="1.5em" style="vertical-align: middle;"}](https://www.vim.org/) [![Neovim](https://img.shields.io/badge/Neovim-57A143.svg?logo=neovim&logoColor=white){height="1.5em" style="vertical-align: middle;"}](https://neovim.io/) [![tmux](https://img.shields.io/badge/tmux-1BB91F.svg?logo=tmux&logoColor=white){height="1.5em" style="vertical-align: middle;"}](https://github.com/tmux/tmux) [![Alacritty](https://img.shields.io/badge/Alacritty-F46D01.svg?logo=alacritty&logoColor=white){height="1.5em" style="vertical-align: middle;"}](https://alacritty.org/)

:::

::: {style="text-align: center;"}

[![GitHub](https://img.shields.io/badge/@i9wa4-181717.svg?logo=github&logoColor=white){height="1.5em" style="vertical-align: middle;"}](https://github.com/i9wa4) [![X](https://img.shields.io/badge/@i9wa4__-000000.svg?logo=x&logoColor=white){height="1.5em" style="vertical-align: middle;"}](https://x.com/i9wa4_) [![Website](https://img.shields.io/badge/🔗_i9wa4.github.io-4285F4.svg?logoColor=white){height="1.5em" style="vertical-align: middle;"}](https://i9wa4.github.io)

:::

:::

### 1.2. 今日のゴール

::: {style="font-size: 1.5em; text-align: center; margin-top: 2em;"}

**AI-Ready な Databricks 開発環境** を紹介する

:::

::: {style="margin-top: 2em;"}

- jupyter-databricks-kernel でローカル開発
- Skinny Notebook Wrapper + Pure Python パターン
- ガードレール (mise + pre-commit + Renovate)

:::

## 2. 課題提起

### 2.1. 従来の Databricks Notebook 開発の課題

![](/assets/2025-12-12-ai-ready-databricks/notebook-challenges.drawio.png)

### 2.2. AI 時代の新たな課題

![](/assets/2025-12-12-ai-ready-databricks/ai-era-challenges.drawio.png)

## 3. 解決策1: jupyter-databricks-kernel

### 3.1. jupyter-databricks-kernel とは

![](/assets/2025-12-12-ai-ready-databricks/jupyter-databricks-kernel.drawio.png)

### 3.2. 開発フロー

::: {style="font-size: 1.2em;"}

1. ローカルで Jupyter を起動
2. コードを書く (Ruff/mypy が使える)
3. セル実行 → Databricks クラスタで実行
4. 結果がローカルに表示

:::

::: {style="margin-top: 1em;"}

- ファイルは自動で Databricks に同期
- クラスタが停止していれば自動起動

:::

## 4. 解決策2: Skinny Notebook Wrapper + Pure Python

### 4.1. プロジェクト構成

![](/assets/2025-12-12-ai-ready-databricks/skinny-notebook-wrapper.drawio.png)

### 4.2. launcher.ipynb は数セル

```python
# Cell 1: Widget定義
dbutils.widgets.text("table_name", "", "Table Name")
dbutils.widgets.text("output_path", "", "Output Path")

# Cell 2: パラメータ取得
table_name = dbutils.widgets.get("table_name")
output_path = dbutils.widgets.get("output_path")

# Cell 3: main() 実行
from main import main
main(table_name=table_name, output_path=output_path)
```

### 4.3. uv による依存関係管理

![](/assets/2025-12-12-ai-ready-databricks/uv-management.drawio.png)

### 4.4. launcher.ipynb (uv対応版)

```python
# Cell 0: 依存関係インストール
%pip install -r requirements.txt

# Cell 1: Widget定義
dbutils.widgets.text("table_name", "", "Table Name")

# Cell 2: パラメータ取得
table_name = dbutils.widgets.get("table_name")

# Cell 3: main() 実行
from main import main
main(table_name=table_name)
```

### 4.5. メリット

::: {style="font-size: 1.2em;"}

| 項目 | 従来 | Skinny Wrapper + uv |
|------|------|-------------------|
| Ruff/mypy | 使えない | 使える |
| Git diff | JSON で読めない | .py で読みやすい |
| テスト | 困難 | pytest で可能 |
| 依存管理 | 手動 | uv.lock で自動 |

:::

## 5. 解決策3: ガードレール

### 5.1. ガードレール構成

![](/assets/2025-12-12-ai-ready-databricks/guardrails.drawio.png)

### 5.2. AI が作ったコードもチェック

::: {style="font-size: 1.2em;"}

- gitleaks: シークレット漏洩検出
- ruff: Python lint/format
- actionlint: GitHub Actions チェック
- ブランチ保護ルール: 最後の砦

:::

::: {style="margin-top: 1em; font-size: 1.1em;"}

AI が pre-commit hook を無視しても、**GitHub Actions で必ずチェック**

:::

## 6. まとめ

### 6.1. AI-Ready 開発環境の全体像

![](/assets/2025-12-12-ai-ready-databricks/ai-ready-overview.drawio.png)

### 6.2. リソース

::: {style="font-size: 1.2em; margin-top: 2em;"}

- jupyter-databricks-kernel

  [github.com/i9wa4/jupyter-databricks-kernel](https://github.com/i9wa4/jupyter-databricks-kernel)

- ガードレール構成 (Zenn記事)

  [zenn.dev/genda_jp/articles/2025-12-06-ai-guardrails-local-cloud](https://zenn.dev/genda_jp/articles/2025-12-06-ai-guardrails-local-cloud)

:::
