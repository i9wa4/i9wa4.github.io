...
![MLエンジニアのワークフロー](/assets/2025-12-22-aeon-tech-hub-databricks/ml-engineer-workflow.drawio.png)

### 5.4. MLワークフローでの権限活用

- Feature Store / Unity Catalog でデータアクセス制御
- MLflow で実験追跡・モデル管理
- Unity Catalog Models でモデルの一元管理
- 本番モデルへのアクセスを適切に制限
...
- `data_dbt_prod` - データチームのdbt本番環境
- `data_dbt_dev` - データチームのdbt開発環境
- `gge_looker_prod` - GGEのLooker本番環境

### 6.2. 運用ルール

- サービスプリンシパルの作成/削除はアカウント管理画面から
- data_adminグループにManager権限を必須付与
- シークレットは1Passwordで管理
- PAT/シークレットは年1回（2/1）更新
