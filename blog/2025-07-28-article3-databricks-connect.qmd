...
前回までの環境に Databricks Connect を追加して、ローカル開発環境から Databricks の Spark クラスタを利用できるようにします。
...
Databricks Connect は、ローカル開発環境から Databricks クラスタに接続してSparkジョブを実行できるライブラリです。
...
uv add 'databricks-connect~=14.3.0'  # 例: DBR 14.3の場合
```
