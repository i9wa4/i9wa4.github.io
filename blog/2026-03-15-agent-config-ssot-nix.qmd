...
パターンファイルのパスは `claude-code.nix` から引数として渡されます。

```{.nix filename="claude-code.nix"}
# repo scripts + generated patterns file を1つの Nix store ディレクトリに合成
...
# home.file (Nix store symlink)
".claude/scripts".source = scriptsDir;
```

スクリプトと生成ファイルが同じディレクトリに配置されるため、パスの管理がシンプルになります。
