...
各ファイルは Markdown で書かれた役割定義で、Nix では管理せず dotfiles リポジトリに直接置いています。
`AGENTS.md` のセクション 9.3 でエージェントの一覧と用途を参照できます。
...
denied-bash-commands・mcp-servers・agent-skills の3つが Nix 管理の設定要素であるのに対し、
`subagents/` はプロンプト定義のため通常の Markdown ファイルとして dotfiles に置き、`AGENTS.md` から参照します。
