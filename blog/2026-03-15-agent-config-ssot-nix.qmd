...
| deny          | No   | true にすると permissions.deny にも追加 (glob は argv から自動導出) |

`deny` を省略すると hook のみで拒否します。拒否時に `justification` のメッセージが表示されるため、Claude は代替手段を理解できます。
`rm` や `sudo` のように絶対に実行させたくないコマンドには `deny = true` を設定し、Claude が計画段階で試行しないようにします。
...
in
{
  inherit entries;
  claudeCode = {
    denyPermissions = ...; # deny = true のエントリのみ (glob は argv から自動導出)
...
# permissions.deny (deny = true のエントリのみ)
...
`home-manager switch` を1回実行すれば、Claude Code の `bash-deny-patterns.sh` (+ `deny = true` があれば `settings.json`) と Codex CLI の `~/.codex/rules/default.rules` に反映されます。
