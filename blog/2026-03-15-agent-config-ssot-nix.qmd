...
Claude Code の hook 用正規表現 (hookRegex) は `argv` から自動導出されます。
...
これにより `home-manager switch` で `~/.claude/bash-deny-patterns.sh` が生成され、PreToolUse hook が source して使います。

```{.nix filename="claude-code.nix"}
bashDenyPatterns = pkgs.writeText "bash-deny-patterns.sh" ''
  DENY_PATTERNS=(
  ${builtins.concatStringsSep "\n" (map (cmd: "  '${mkHookRegex cmd}'") deniedBash)}
  )
  DENY_JUSTIFICATIONS=(
...
`claudeGlob` を持つエントリだけが `permissions.deny` にも追加されます。

```{.nix filename="claude-code.nix"}
permissions = {
  deny = (map (cmd: "Bash(${cmd.claudeGlob})") (builtins.filter (cmd: cmd ? claudeGlob) deniedBash)) ++ [
    "Read(**/*key*)"
    "Read(.env*)"
    # ...
  ];
...
Codex CLI 側 (`codex-cli.nix`) では、`argv` と `justification` を読み取ります。
...
同一ソースから複数の異なるフォーマットへ変換しています。
`home-manager switch` を1回実行するだけで、Claude Code の `bash-deny-patterns.sh` (+ `claudeGlob` があれば `settings.json`) と Codex CLI の `~/.codex/rules/default.rules` に同じルールが反映されます。
