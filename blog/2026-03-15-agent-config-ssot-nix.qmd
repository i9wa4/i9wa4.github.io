...
| フィールド    | 用途                                                           |
| ------------- | -------------------------------------------------------------- |
| claudeGlob    | Claude Code の permissions.deny 用 glob パターン (事前抑止)    |
| hookRegex     | Claude Code の PreToolUse hook 用正規表現 (実行時検査)         |
| argv          | Codex CLI の prefix_rule 用トークン配列                        |
| justification | 拒否時に表示するメッセージ (Claude Code hook + Codex CLI 共通) |

Claude Code では2層の防御として機能します。

`claudeGlob` は事前抑止層です。`permissions.deny` に変換され、Claude がコマンドを計画する時点で「試行しない」と判断します。
...
`hookRegex` と `justification` は実行時検査層です。`home-manager switch` で `~/.claude/bash-deny-patterns.sh` として生成され、PreToolUse hook が source して使います。
...
同一ソースから3つの異なるフォーマットへ変換しています。
`home-manager switch` を1回実行するだけで、Claude Code の `settings.json`・`bash-deny-patterns.sh` と Codex CLI の `~/.codex/rules/default.rules` の3箇所に同じルールが反映されます。
