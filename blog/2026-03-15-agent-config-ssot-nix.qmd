...
| 生成先                               | 生成内容                                              |
| ------------------------------------ | ----------------------------------------------------- |
| `~/.claude/scripts/bash-deny-patterns.sh`    | DENY_PATTERNS に `'git.*push'`、DENY_JUSTIFICATIONS に `'pushing is denied'` を追加 |
...
| 生成先                               | 生成内容                                              |
| ------------------------------------ | ----------------------------------------------------- |
| `~/.claude/scripts/bash-deny-patterns.sh`    | DENY_PATTERNS に `'\brm\b'`、DENY_JUSTIFICATIONS に `'rm is denied; use mv /tmp/ instead'` を追加 |
...
#### PreToolUse hook スクリプト

Claude Code は `settings.json` の `hooks.PreToolUse` で Bash コマンド実行前にスクリプトを呼び出します。
このスクリプトが `bash-deny-patterns.sh` を source し、コマンドをパターンマッチで検査します。

```{.bash filename="claude-pretooluse-bash-deny.sh"}
#!/usr/bin/env bash
set -o errexit
set -o nounset
set -o pipefail
...
スクリプトは `dirname $0` で自身のディレクトリを解決し、同じディレクトリにある `bash-deny-patterns.sh` を source します。
引数でパスを渡す必要はありません。
