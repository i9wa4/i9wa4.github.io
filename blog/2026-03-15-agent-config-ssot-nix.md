...
そこで Nix home-manager を使って設定要素を SSOT (Single Source of Truth)
化しました。 設定は `nix/home-manager/agents/`
ディレクトリ以下にまとめています。
...
に集約されているため、新しいエントリの追加はデータを追加するだけです。
...
`subagents/` には Claude Code の Agent
ツールで呼び出せる専門調査エージェントの役割定義が入っています。

各ファイルは YAML フロントマター (`name`, `description`, `model`) と
Markdown 本文で構成されています。 `subagents/*.md`
が単一のソースで、ツールごとに異なる形式に変換されます。

- `claude-code.nix` は `subagents/` を `~/.claude/agents/`
  にシンボリックリンクし、生の `.md` ファイルを Claude Code に渡します。
- `codex-cli.nix` は Nix ビルド時に各 `.md` を `.toml`
  に変換し、`~/.codex/agents/` に配置します。`model`
  フィールドは削除されるため、エージェントは親セッションのモデルを継承します。
...
`subagents/` は denied-bash-commands・mcp-servers・agent-skills と同様に
Nix が管理する要素です。 `subagents/*.md` を1箇所更新して
`home-manager switch` を実行するだけで、Claude Code と Codex CLI
の両方に反映されます。
