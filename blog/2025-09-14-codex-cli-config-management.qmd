...
共通設定だけ dotfiles の管理対象とし、プロジェクト別の設定はスクリプトで静止してしまうというものです。
...
cp -f "$COMMON" "$OUTPUT"
sed -i '' '/# PROJECT_SETTINGS_PLACEHOLDER/d' "$OUTPUT"

find "$GHQ_BASE" -maxdepth 3 -type d -name ".git" 2>/dev/null | sed 's/\/\.git$//' | sort | while read -r repo; do
  [[ -z $repo ]] && continue

  echo "
[projects.\"$repo\"]
trust_level = \"trusted\"" >>"$OUTPUT"

  case "$repo" in
    */dotfiles) echo 'approval_policy = "never"' >>"$OUTPUT" ;;
    */prod*) echo 'sandbox_mode = "read-only"' >>"$OUTPUT" ;;
  esac
done
