---
title: "Claude Code と Codex CLI のアップデート情報収集術"
author: uma-chan
categories:
  - "blog"
  - "tech-ai"
...
Contents API に Raw メディアタイプを指定すると、ファイル内容をそのまま取得できます。
...
### 5.4. codex-release-check スキル

以下のブロックを `~/.claude/skills/codex-release-check/SKILL.md` にコピーして保存します。

```{.sh filename="terminal"}
mkdir -p ~/.claude/skills/codex-release-check/references
```
