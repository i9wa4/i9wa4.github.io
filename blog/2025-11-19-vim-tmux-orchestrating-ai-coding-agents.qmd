...
## 3. Vim と tmux で複数のAIエージェントを操作する様子

構築後の操作を先にお見せしますと Claude Code * 1 と Codex CLI * 2 に同じカスタム指示を送る実演動画が以下となります。
...
Vim から tmux 経由で AI エージェントにプロンプトを送信するためのプラグインです。

送信先：

- (デフォルト) 同一ウインドウの中で最も小さい番号のペインに送信
- 同一ウインドウの特定のペインにのみ送信
- 同一ウインドウの全てのペインの AI エージェントに送信
