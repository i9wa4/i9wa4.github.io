...
AI コーディングエージェントに任せる作業が増えると、毎回似たようなことを頼んでいることに気づきます。
...
そこで、繰り返し必要になる判断材料を、再利用できる運用知識として外に出します。
Agent Skills は、その運用知識を小さなまとまりとして持つ方法の 1 つです。
...
ここで混ぜないようにしたいのが、仕様、カタログ、検証ツール、配布ツールです。
...
Agent Skills を入れても、AI の出力が完全に決定的になるわけではありません。
