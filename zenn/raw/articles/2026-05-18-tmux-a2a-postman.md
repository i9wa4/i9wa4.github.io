...
1 つのエージェントに実装、調査、レビューまで抱えさせると、コンテキストも判断もすぐ重くなりがちです。役割を分けると各エージェントが狭い範囲に集中でき、レビューや確認を実装とは独立した視点で残せます。
...
tmux のペインと Markdown の受け渡しを使うので、特定ベンダーのネイティブなマルチエージェント機能に寄せなくても組み込めます。Claude Code や Codex CLI のような CLI/TUI 型の AI コーディングエージェントを、同じ作業場の別ノードとして扱いやすいのが実用上の利点です。
