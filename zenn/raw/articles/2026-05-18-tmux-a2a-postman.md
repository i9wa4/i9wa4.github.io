...
受信側が読む mail には、依頼本文だけでなく、`postman.md` で設定した共通ルール、役割、経路、返答の期待値も入ります。長めのタスクでも、エージェントが自分の担当や返すべき形を毎回確認しやすくなります。
...
`🔷` は、そのノードに未解決の reply-required な受信依頼がある状態です。`🟡` は、そのノードが送った reply-required な依頼の返事待ち、`🟢` は未処理の依頼や返事待ちがない状態、`⚫` は初期状態を表します。
...
長い作業の途中で、担当がずれる、reply-required な依頼を閉じ忘れる、使えるローカル手順を忘れる、ということが起きます。

tmux-a2a-postman では、そこを三つに分けて残します。

`postman.md` 由来の役割、経路、返答の期待値は受信 mail に入ります。reply-required な依頼は、閉じていないループとして状態に残ります。Agent Skills のカタログは、使える Skill 名と短い説明を役割へ渡し、エージェントが適切な Agent Skills を選びやすくします。

つまり、誰として動くか、何を返す必要があるか、どのローカル手順を使えるかを、ターミナルの記憶だけに寄せないようにしています。
...
名前の `a2a` は agent-to-agent の受け渡しという意味で使っています。ただし現時点では、A2A Protocol 準拠のサーバーではありません。
