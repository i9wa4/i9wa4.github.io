---
title: "Claude Code/Codex CLIをtmuxで並べるならMarkdownの郵便受けを足す"
emoji: "🐴"
type: "tech"
topics:
  - "tmux"
...
Zenn でも、Claude Code を tmux で多重起動する話、Agent Teams の作業状況を画面分割で見る話、Codex CLI を tmux 越しに扱うハーネスの話が増えています。

方向性はよく分かります。1 体のエージェントに順番に頼むより、役割を分けて並べたほうが速い場面があるからです。

ただ、実際に並べると次に困るのは「どう起動するか」より後の話です。
...
今回の話は、その作業場をどう作るかではありません。

その上に載る依頼、返信待ち、レビュー、証拠をどう扱うかです。

tmux 自体は、エージェント間の依頼や返信待ちを管理する道具ではありません。
...
## 3. tmux-a2a-postman が足すもの

`tmux-a2a-postman` は、ペインを増やす道具ではありません。

既にある tmux セッションと CLI エージェントの上に、次のものを足します。

- Markdown の手紙として残る依頼
- inbox と read archive による未読と既読
- 返信が必要な依頼を閉じる input request
- role ごとの通信ルールと責務
- DONE や BLOCKED に添える証拠の置き場

ポイントは、協調の状態をローカルのファイルとして残すことです。
