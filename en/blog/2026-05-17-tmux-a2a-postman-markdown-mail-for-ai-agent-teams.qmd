...
That matters because postman does not prescribe a single team shape. You can define roles, routes, reply expectations, and operating rules in Markdown, then revise them as the workflow changes. A postman-oriented Agent Skill such as `postman-config-auditor` gives agents local knowledge for auditing that topology and suggesting scoped refinements, so the harness can become more tailored without the user explaining every rule each time.
...
This is where the Agent Skills catalog becomes operational rather than decorative. A receiver that has `postman-session-operator` in its relevant Agent Skills can know when to claim mail, how to treat the archive, how to close an exact input request, and when to report blocked or delivery trouble. A config-focused role with `postman-config-auditor` can inspect the topology and help refine roles, routes, and rules as the user's preferred workflow evolves.

Agent Skills do not make the agent magically autonomous. They provide local operating knowledge. The task mail still carries the current constraints, reply policy, success checks, and artifact references, especially for long-running implementation, review, or approval work.
...
- Agent Skills or local instructions for agent behavior
...
This article is not the full setup manual. The [README quick start](https://github.com/i9wa4/tmux-a2a-postman#quick-start) is still the right place for installation, complete config, and edge cases. But the first runnable shape should be easy to picture:
...
The same state is available through `get-status` for full inspection or `get-status-oneline` for a status-bar-sized heartbeat. That is useful when a review cycle is running: the operator can see that mail is moving, a reply is still open, or a reviewer has returned evidence-backed feedback without treating the agent panes as the source of truth.
