...
description: |
  ...
  tmux-a2a-postman is my small local mailbox layer for that gap. It complements native agent features when I want durable Markdown instructions, explicit handoffs, tool-neutral coordination, and inspectable local state.
...
This is the part I usually want outside a native model feature. A runtime may provide its own subagents or task tools, but `postman.md` is a local Markdown contract I can review in Git and apply across terminal tools.
...
## 5. The Minimal Mail Loop

The operational loop is intentionally small.

One role sends Markdown mail:

```{.bash}
tmux-a2a-postman send-heredoc --to worker --reply-required <<'POSTMAN_BODY'
Tighten the parser behavior described in the task note.

Constraints:
- Keep the change scoped.
- Run the relevant tests.
- Reply with DONE or BLOCKED and evidence.
POSTMAN_BODY
```
...
That is the core. The message body is Markdown. The shell surface is a quoted heredoc so code fences, variables, backticks, and YAML survive the send. `pop` moves unread mail into a read archive and returns the archived Markdown path. A receiver is expected to read that archived body before acting.

For required replies, postman opens an exact input request. The reply closes that transport slot, which means the sender can tell which `DONE` or `BLOCKED` answered which request. The README covers the full command syntax; the article-level point is the state model: delivered mail, claimed mail, archived body, and exact reply closure.

That changes where instructions live. In a long-running agent conversation, earlier constraints can become distant. With mail, each handoff can carry the local instructions for that moment: constraints, reply policy, success checks, and references to the task artifact.
