...
This is not a chat UI feature, and the daemon TUI does not render a transcript like the one below. It is only a fictional way to make the route concrete; role names vary by configuration, and `guardian` here is just a review-gate example:

```{.text}
user> Please tighten the parser explanation and validate the page.
...
orchestrator> guardian: review the result when worker is done.
worker> DONE: edited the section and rendered the page.
guardian> APPROVED: evidence matches the requested scope.
