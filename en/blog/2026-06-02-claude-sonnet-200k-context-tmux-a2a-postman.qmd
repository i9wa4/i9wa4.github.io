...
description: |
  Claude Sonnet is the practical workhorse when Opus is quota-expensive, but long context still needs workflow discipline. Learn how tmux-a2a-postman turns agent work into explicit mail, task artifacts, compaction recovery, and smaller agent jobs. #ClaudeCode #AIAgents #tmux #DeveloperTools
...
They also matter because Sonnet is the model I expect to live in for most sustained agent runs. Opus is valuable for planning difficult work, debugging thorny failures, and making architecture decisions, but current Claude usage guidance makes it a model to reach for deliberately, not something to leave on by default.

The trap is treating that window as a workflow.

Anthropic's [context window documentation](https://platform.claude.com/docs/en/build-with-claude/context-windows) makes the operational warning clear: more context is not automatically better, and recall can degrade as token count grows. Current Claude docs also show why "Sonnet context" is not one fixed number. The [paid-plan context page](https://support.claude.com/en/articles/8606394-how-large-is-the-context-window-on-paid-claude-plans) currently describes 500K chat windows for recent Opus models and Sonnet 4.6, 200K windows outside those models, and 1M Claude Code paths for recent Opus models and Sonnet 4.6 with usage-credit requirements in some cases. The [API context page](https://support.claude.com/en/articles/8606395-how-large-is-the-claude-api-s-context-window) similarly separates 1M-capable recent Opus and Sonnet 4.6 deployments from 200K-class models. The exact limit depends on the model, product surface, and deployment, but the operating problem does not go away when the window gets larger.
...
That usage model is where Opus versus Sonnet matters. The [usage best-practices page](https://support.claude.com/en/articles/9797557-usage-limit-best-practices) tells paid users to monitor weekly limits for Opus and for all other models. The [Max plan page](https://support.claude.com/en/articles/11049741-what-is-the-max-plan) says Max has one weekly limit across all models and another for Sonnet models only. Claude Code's [model and limit guide](https://support.claude.com/en/articles/14552983-models-usage-and-limits-in-claude-code) calls Sonnet the default for most coding work, says Opus uses meaningfully more quota, and suggests planning with Opus and executing with Sonnet.

In practical terms, Sonnet is where most sustained work has to become efficient. A 200K-class or larger Sonnet context is useful for tasks that are too large for casual chat but should not spend Opus quota on every turn. Examples include a repository plus logs, a design document plus implementation files, a long debugging transcript, or a review packet with enough source context to reason safely. [Max 5x and Max 20x](https://support.claude.com/en/articles/11049741-what-is-the-max-plan) increase usage capacity over Pro, but they do not remove the need to manage context shape. A bigger window helps; it also makes each messy conversation more expensive against the same shared usage system.
