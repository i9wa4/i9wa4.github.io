...
A large context window is still working memory. Anthropic's [context window documentation](https://platform.claude.com/docs/en/build-with-claude/context-windows) makes the operational warning clear: more context is not automatically better, and recall can degrade as token count grows. The same page currently lists Claude Sonnet 4.6 as a 1M-token model on Claude API, Amazon Bedrock, and Vertex AI, while Claude Sonnet 4.5 and Sonnet 4 are in the 200k-token class. The exact limit depends on the model, product surface, and deployment, but the operating problem does not go away when the window gets larger.

That is also why 200K still matters. Current Claude Help Center pages separate [usage limits from length limits](https://support.claude.com/en/articles/11647753-how-do-usage-and-length-limits-work): usage limits are the plan budget over time, while length limits are the context window for a single conversation. The current [paid-plan context page](https://support.claude.com/en/articles/8606394-how-large-is-the-context-window-on-paid-claude-plans) describes 500K chat windows for several current Opus models and Sonnet 4.6, 200K windows outside those models, and 1M Claude Code paths for current Opus and Sonnet models with usage credit requirements in some cases. The [API context page](https://support.claude.com/en/articles/8606395-how-large-is-the-claude-api-s-context-window) similarly separates 1M-capable current Opus and Sonnet deployments from 200K-class models. Pro and Max usage is also shared across `claude.ai`, Claude Desktop, and Claude Code.

In practical terms, a 200K-token model is useful for work that is too large for casual chat but does not justify, qualify for, or fit within a 1M-token path. Examples include a repository plus logs, a design document plus implementation files, a long debugging transcript, or a review packet with enough source context to reason safely. [Max 5x and Max 20x](https://support.claude.com/en/articles/11049741-what-is-the-max-plan) increase usage capacity over Pro, but they do not remove the need to manage context shape. A bigger window helps; it also makes each messy conversation more expensive against the same shared usage system.
