...
description: |
  I moved my lazygit commit-message helper from `claude -p` to `codex exec`.
...
The goal was to make the commit flow feel faster without turning it into an automatic commit button:

- Start a shell script from a lazygit custom command
- Generate a commit-message draft in the background
- Immediately start `git commit`
- Let pre-commit hooks and message generation run at the same time
- Open the editor at the end so I can review the message before committing

In the current version, the non-interactive AI command is Codex CLI's `codex exec`.
